"""
Bybit Monitor ASYNC — ОПТИМИЗИРОВАННАЯ ВЕРСИЯ
...
BYBIT_FUTURES_REST_URL = "https://api.bybit.com/v5/market/orderbook?category=linear&symbol={}USDT&limit=100"
BYBIT_SPOT_REST_URL = "https://api.bybit.com/v5/market/orderbook?category=spot&symbol={}USDT&limit=100"
...
async def init_order_book_async(symbol, market='futures', log_func=print):
    try:
        ...
        log_func(f"✅ bybit {market} {symbol}: {len(bids)} bids, {len(asks)} asks | плотностей: {saved_count}")
        return saved_count
    except Exception as e:
    ...
    try:
        if market == 'futures':
            async with bybit_futures_lock:
                ...
                ts = dict(bybit_futures_density_timestamps.get(symbol, {}))
                stats = dict(bybit_futures_volume_stats.get(symbol, {}))
                if not book:
                    return 0
        else:
            async with bybit_spot_lock:
                book = bybit_spot_order_books.get(symbol, {})
                ts = dict(bybit_spot_density_timestamps.get(symbol, {}))
                stats = dict(bybit_spot_volume_stats.get(symbol, {}))
                if not book:
                    return 0
    ...
    while True:
        try:
            ...
            if not symbols:
                ...
                continue
            log_func(f"🔌 bybit {market} WS: {len(symbols)} символов")
            ...
            log_func(f"⚠️ bybit {market} WS закрыт, reconnect 3 сек")
            await asyncio.sleep(3)
        except Exception as e:
            log_func(f"❌ bybit {market} WS ошибка: {e}, reconnect 5 сек")
            await asyncio.sleep(5)
...
async def periodic_refresh(market='futures', log_func=print):
    ...
    while True:
        ...
        try:
            ...
            log_func(f"🔄 bybit {market}: ротация, активных {len(new_active)}")
        except Exception as e:
            log_func(f"❌ Ошибка periodic_refresh(bybit {market}): {e}")

async def main_async(log_func=print):
    global bybit_futures_symbols, bybit_spot_symbols, stable_futures_symbols, stable_spot_symbols
    log_func("🚀 Запуск Bybit Async Monitor...")
    stable_f = await get_stable_coins_async('linear', STABLE_COINS_LIMIT)
    ...
    log_func(f"✅ Bybit Monitor: {len(active_futures)} futures, {len(active_spot)} spot")
    tasks = [
