"""
OKX Monitor — мониторинг плотностей OKX Futures и Spot
"""
import json
import time
import threading
...
def get_top_symbols(limit=30):
    """Отбор OKX Futures по гибридной формуле (RVOL+NATR+%)"""
    try:
        exchange = ccxt.okx({
        ...
        # OKX swap: volCcy24h в базовой валюте → пересчитываем в USDT
...
def get_top_spot_symbols(limit=30):
    """Отбор OKX Spot по гибридной формуле (RVOL+NATR+%)"""
    try:
        exchange = ccxt.okx({
            'enableRateLimit': True,
            'timeout': 10000,
        ...
        print(f"❌ Ошибка в get_top_spot_symbols(okx spot): {e}")
        return []
...
def update_spot_order_book(symbol, bids_delta, asks_delta, log_func=print):
    ...
    try:
        ...
        if changed:
            ...
            if key not in okx_spot_last_sync_time or (now - okx_spot_last_sync_time[key]) >= 3:
