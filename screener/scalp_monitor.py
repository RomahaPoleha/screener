...
def init_order_book(symbol):
    ...
    try:
        ...
        log(f"❌ Ошибка init_order_book({symbol}): {e}")
        return False
...
def start_websocket_connection(symbol_group, symbols_list):
    ...
    try:
        ...
        log(f"❌ Ошибка start_websocket_connection({symbol_group}): {e}")
        log(traceback.format_exc())


def start_all_websocket_connections():
    """Запустить все WebSocket подключения"""
    global symbols
