...
def get_top_spot_symbols(limit=30):
    ...
    try:
        ...
        tickers = exchange.fetch_tickers()
        print(f"🔍 Gate Spot: получено {len(tickers)} тикеров")
        ...
        print(f"📊 Gate Spot: отсеяно по объёму {passed_volume}, кандидатов {len(candidates)}")

        candidates.sort(key=lambda x: x[1], reverse=True)
        return [s[0] for s in candidates[:limit]]

    except Exception as e:
        print(f"❌ Ошибка в get_top_spot_symbols(gate): {e}")
        return []


def init_spot_order_book(symbol, log_func=print):
    """Инициализация стакана Gate Spot через REST"""
    try:
        url = GATE_SPOT_REST_URL.format(symbol)
        res = requests.get(url, timeout=10, headers={'User-Agent': 'Mozilla/5.0'})

        if not res.ok:
            log_func(f"⚠️ gate spot {symbol}: HTTP {res.status_code}")
            return 0

        try:
            data = res.json()
        except Exception:
            log_func(f"⚠️ gate spot {symbol}: не JSON: {res.text[:200]}")
            return 0

        if not isinstance(data, dict) or ('asks' not in data and 'bids' not in data):
            log_func(f"⚠️ gate spot {symbol}: неожиданный формат")
            return 0
        ...
        if not bids and not asks:
            log_func(f"⚠️ gate spot {symbol}: пустой стакан")
            return 0

        with gate_spot_order_books_lock:
        ...
        log_func(
            f"✅ gate spot Стакан {symbol}: "
            f"{len(bids)} bids, {len(asks)} asks | плотностей: {saved_count}"
        )
        return saved_count

    except requests.exceptions.Timeout:
        log_func(f"❌ init_spot_order_book(gate spot {symbol}): timeout")
        return 0
    except Exception as e:
        log_func(f"❌ init_spot_order_book(gate spot {symbol}): {e}")
        return 0
...
def sync_spot_to_cache(symbol, log_func=print):
    try:
        ...
        log_func(f"❌ sync_spot_to_cache(gate spot {symbol}): {e}")
        return 0


def update_spot_order_book(symbol, bids_delta, asks_delta, log_func=print):
    global gate_spot_last_sync_time
    ...
    try:
        ...
        if changed:
            now = time.time()
            key = f"gate:spot:{symbol}"
            if key not in gate_spot_last_sync_time or (now - gate_spot_last_sync_time[key]) >= 10:
                sync_spot_to_cache(symbol, log_func)
                gate_spot_last_sync_time[key] = now

    except Exception as e:
        log_func(f"❌ update_spot_order_book(gate spot {symbol}): {e}")
...
def process_spot_message_queue(log_func=print):
    ...
    while True:
        try:
            ...
            if data.get('channel') != 'spot.order_book_update':
                continue
            ...
            pair = result.get('c') or result.get('currency_pair')
            if not pair:
                continue
            ...
            log_func(f"❌ gate spot Ошибка обработки: {e}")
...
def on_spot_open(ws):
    ...
    pairs = [f"{s}_USDT" for s in ws.symbols]
    subscribe_msg = {
        "time": int(time.time()),
    ...
    while not gate_spot_ws_stop_event.is_set():
        ...
        try:
            ws = websocket.WebSocketApp(
                ...
                on_error=lambda ws, err: log_func(f"❌ gate spot WebSocket ошибка: {err}"),
                on_close=lambda ws, code, msg: log_func(f"⚠️ gate spot WebSocket закрыт: {code} {msg}")
            )
...
def refresh_gate_spot_symbols(log_func=print):
    ...
    log_func("🔄 Обновление списка Gate Spot...")

    old_symbols = set(gate_spot_symbols)
    candidates = get_top_spot_symbols(60)
    ...
    for symbol in candidates:
        ...
        if saved_count > 0:
            new_active.append(symbol)
            log_func(f"✅ gate spot {symbol}: добавлен (плотностей: {saved_count})")
        else:
            log_func(f"⚠️ gate spot {symbol}: пропущен (нет плотностей > $10K)")
    ...
    if removed:
        with gate_spot_order_books_lock:
            for symbol in removed:
                gate_spot_order_books.pop(symbol, None)
                gate_spot_density_timestamps.pop(symbol, None)
        log_func(f"🗑️ gate spot удалены: {', '.join(sorted(removed))}")

    gate_spot_symbols = new_active

    if removed or added:
        log_func(f"🔄 gate spot: добавлено {len(added)}, удалено {len(removed)}")

        gate_spot_ws_stop_event.set()

        if gate_spot_ws_instance:
            try:
                gate_spot_ws_instance.close()
            except Exception:
                pass

        time.sleep(2)
        gate_spot_ws_stop_event.clear()
        ...
        log_func("✅ gate spot: список не изменился")
